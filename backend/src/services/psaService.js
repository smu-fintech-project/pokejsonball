...
async function fetchPSACertificate(certNumber, attempt = 0) {
  ...
  try {
    ...
    if (error.response) {
      ...
      switch (status) {
        case 404:
          console.warn(`⚠️  PSA cert not found: ${certNumber}`);
          throw {
            code: 'CERT_NOT_FOUND',
        ...
        case 429:
          ...
          if (attempt < PSA_API_CONFIG.maxRetries) {
            ...
            console.warn(`⚠️  PSA rate limit hit, retrying in ${delay}ms (attempt ${attempt + 1}/${PSA_API_CONFIG.maxRetries})`);
            await sleep(delay);
        ...
        case 503:
          ...
          if (attempt < PSA_API_CONFIG.maxRetries) {
            const delay = getBackoffDelay(attempt);
            console.warn(`⚠️  PSA server error, retrying in ${delay}ms (attempt ${attempt + 1}/${PSA_API_CONFIG.maxRetries})`);
            await sleep(delay);
            return fetchPSACertificate(certNumber, attempt + 1);
          }
    ...
    if (attempt < PSA_API_CONFIG.maxRetries && !error.code?.startsWith('CERT_')) {
      const delay = getBackoffDelay(attempt);
      console.warn(`⚠️  PSA network error, retrying in ${delay}ms (attempt ${attempt + 1}/${PSA_API_CONFIG.maxRetries})`);
      await sleep(delay);
      return fetchPSACertificate(certNumber, attempt + 1);
    }
...
export function clearPSACache(certNumber = null) {
  ...
  console.log(`🗑️  Clearing PSA cache for: ${certNumber || 'all'}`);
}
